...
$(document).ready(function() {
	...
	$('#add-client').on('click',function(event){
		$('.table-index').hide();
		$.get("client/new",function(data){
		$('.card-body').html(data);
		});

		$(this).hide();
		$("div .card-body").removeClass("mx-auto");
		$(".card-header:contains('Home')").append('<a class="al btn btn-outline-secondary mr-3 float-right" href="/client">Clients</a>');
	});


	$("a:contains('clients')").on("click",function(){
...
$("table tbody td button.edit").on("click",function(){
		...
		if(hint.prop("disabled") == true){
			...
			if( index == 1 ){
				...
				var link = "http://"+window.location.host+'/client/'+$(td).parent().attr("id_attr") ;
	...
	$(".search-button[type='submit']").on('click',function(event){
			...
			$.post('client/search',{
				'designation':$('.search-box').val(),

			},function(data){
				if(data){

					var link = "http://"+window.location.host+'/client/'+data.result.id ;
					...
					td.eq(1).html("<a href='"+link+"'>"+data.result.designation+"</a>");
